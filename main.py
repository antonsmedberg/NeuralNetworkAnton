...
def main():
    ...
    try:
        ...
        if config["use_synthetic_data"]:
            ...
            X_train, X_test, y_train, y_test = generate_synthetic_data('classification',
                                                                       n_samples=config["n_samples"],
                                                                       n_features=config["n_features"],
                                                                       n_classes=config["n_classes"],
                                                                       random_state=config["random_state"])
        else:
            X_train, X_test, y_train, y_test = load_real_data(config["filepath"], config.get("target_column"))
